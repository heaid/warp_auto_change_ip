...
if [[ "$input" == "n" ]];then
    bash <(curl -fsSL https://github.com/P3TERX/warp.sh/blob/main/warp.sh) 4
elif [[ "$input" == "y" ]];then
    read -r -p "Input the region you want(e.g. HK,SG):" area
fi
while [[ "$input" == "y" ]]
do
    result=$(curl --user-agent "${UA_Browser}" -fsL --write-out %{http_code} --output /dev/null --max-time 10 "https://www.netflix.com/title/81280792" 2>&1)
    if [[ "$result" == "404" ]];then
